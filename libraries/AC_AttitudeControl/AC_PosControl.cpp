...
void AC_PosControl::run_z_controller()
{
    ...
    if (_flags.reset_accel_to_throttle) { EXECUTE_MARK();
        // Reset Filter
        _accel_error.z = 0;
    ...
    if ((!_motors.limit.throttle_lower && !_motors.limit.throttle_upper) || (i>0&&_accel_error.z<0) || (i<0&&_accel_error.z>0)) { EXECUTE_MARK();
