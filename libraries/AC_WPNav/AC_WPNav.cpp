...
bool AC_WPNav::advance_wp_target_along_track(float dt)
{
    ...
    if (is_positive(kP)) {   EXECUTE_MARK(); // avoid divide by zero
...
bool AC_WPNav::set_spline_origin_and_destination(const Vector3f& origin, const Vector3f& destination, bool terrain_alt, bool stopped_at_start, spline_segment_end_type seg_end_type, const Vector3f& next_destination)
{
    ...
    if (stopped_at_start || !prev_segment_exists) { EXECUTE_MARK();
        ...
        if (_flags.segment_type == SEGMENT_STRAIGHT) { EXECUTE_MARK();
            ...
            if (_spline_time > 1.0f && _spline_time < 1.1f) {   EXECUTE_MARK();  // To-Do: remove hard coded 1.1f
                _spline_time -= 1.0f;
            }else{ EXECUTE_MARK();
                _spline_time = 0.0f;
            }
...
bool AC_WPNav::get_vector_NEU(const Location_Class &loc, Vector3f &vec, bool &terrain_alt)
{
    ...
    // convert altitude
    if (loc.get_alt_frame() == Location_Class::ALT_FRAME_ABOVE_TERRAIN) { EXECUTE_MARK();
        int32_t terr_alt;
        if (!loc.get_alt_cm(Location_Class::ALT_FRAME_ABOVE_TERRAIN, terr_alt)) { EXECUTE_MARK();
            return false;
        }
        vec.z = terr_alt;
        terrain_alt = true;
        ...
        terrain_alt = false;
        int32_t temp_alt;
        if (!loc.get_alt_cm(Location_Class::ALT_FRAME_ABOVE_ORIGIN, temp_alt)) { EXECUTE_MARK();
            return false;
        }
        vec.z = temp_alt;
        terrain_alt = false;
