...
bool Copter::ModeAcro::init(bool ignore_checks)
{
   ...
   if (motors->armed() && ap.land_complete && !copter.flightmode->has_manual_throttle() &&
           (get_pilot_desired_throttle(channel_throttle->get_control_in(), copter.g2.acro_thr_mid) > copter.get_non_takeoff_throttle())) { EXECUTE_MARK();
       return false;
   }
...
void Copter::ModeAcro::run()
{
    ...
    if (!motors->armed() || ap.throttle_zero || !motors->get_interlock()) { EXECUTE_MARK();
...
void Copter::ModeAcro::get_pilot_desired_angle_rates(int16_t roll_in, int16_t pitch_in, int16_t yaw_in, float &roll_out, float &pitch_out, float &yaw_out)
{
    ...
    if (g.acro_trainer != ACRO_TRAINER_DISABLED) { EXECUTE_MARK();
        ...
        if (g.acro_trainer == ACRO_TRAINER_LIMITED) { EXECUTE_MARK();
            if (roll_angle > aparm.angle_max){ EXECUTE_MARK();
                rate_ef_level.x -=  g.acro_balance_roll*(roll_angle-aparm.angle_max);
            }else if (roll_angle < -aparm.angle_max) { EXECUTE_MARK();
                rate_ef_level.x -=  g.acro_balance_roll*(roll_angle+aparm.angle_max);
            }

            if (pitch_angle > aparm.angle_max){ EXECUTE_MARK();
                rate_ef_level.y -=  g.acro_balance_pitch*(pitch_angle-aparm.angle_max);
            }else if (pitch_angle < -aparm.angle_max) { EXECUTE_MARK();
                rate_ef_level.y -=  g.acro_balance_pitch*(pitch_angle+aparm.angle_max);
            }
        }
